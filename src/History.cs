...
namespace TabbedExplorer
{
    ...
    internal static class History
    {
        ...
        ///从老的 `ContextMenu`/`MenuItem` 换过来：那条路上**点条目不触发 Click**
        /// （用户报的「历史记录点开后所有功能都不可用」），原因见 `PopMenu` 类注释。
        /// `open == null` 的条目就是灰着的标题/说明行。
        /// `openManager` = 「打开历史记录管理器」（用户把原来那条「打开历史记录文件」换成了它 ——
        /// 直接把 json 丢给记事本太糙，管理器里能搜、能挑、能手删）。
        ///
        /// 用户追加：**按日期归类** —— 同一堆的前面插一条灰标题（今天 / 昨天 / …）。
        /// </summary>
        public static PopItem[] BuildMenu(Action<string> open, Action openManager)
        {
            ...
            r.Add(PopMenu.Split());
            // 用户：「打开历史记录文件」改成「打开历史记录管理器」（界面模仿书签管理器）。
