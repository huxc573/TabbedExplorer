...
namespace TabbedExplorer
{
    ...
    internal static class Theme
    {
        ...
        ///
        /// 用户：原生资源管理器激活与未激活的颜色是不一样的，我们的外壳要跟着模拟
        ...
        public static bool Reload()
        {
            ...
            if (dark)
            {
                ...
                // 取同一个色，两块拼起来才是连续的一整片（用户：标签也不够黑，要和查看那一页一样黑）。
        ...
        /// 把窗口的标题栏（非客户区）刷成深/浅色。
        ///
        /// ⚠ 光调 `DwmSetWindowAttribute` 经常**当时看不出效果** —— 用户报的就是这个：
        ...
        /// <summary>
        /// 让一个 `ToolTip` 的背景色 / 字体颜色跟着颜色模式走（用户报的 bug）。
        ///
        ...
        ///     只按一行算高度 ⇒ 提示框太矮 ⇒ 下半截被切（用户报的「悬停显示不全」就是这个）。
